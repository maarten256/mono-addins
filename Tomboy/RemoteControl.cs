...
namespace Tomboy
{
	...
	public class RemoteControl : IRemoteControl
	{
		...
		// Required by Tmds.DBus to identify the object on the bus
		public ObjectPath ObjectPath => _objectPath;

		public RemoteControl(NoteManager manager, string objectPath)
		{
			_objectPath = new (objectPath);
			_noteManager = manager;
			_noteManager.NoteDeleted += OnNoteDeleted;
			_noteManager.NoteAdded += OnNoteAdded;
			_noteManager.NoteSaved += OnNoteSaved;
		}
		...
		public bool DisplayNote(string uri)
		{
			...
			note = _noteManager.FindByUri(uri);
			if (note == null)
				return false;
		...
		{
			Note note = _noteManager.Find(linked_title);
			return (note == null) ? String.Empty : note.Uri;
		}

		public string FindStartHereNote()
		{
			Note note = _noteManager.FindByUri(NoteManager.StartNoteUri);
			return (note == null) ? String.Empty : note.Uri;
		}

		public string CreateNote()
		{
			if (_noteManager.ReadOnly)
				return string.Empty;
			try
			{
				Note note = _noteManager.Create();
		...
		public string CreateNamedNote(string linked_title)
		{
			...
			note = _noteManager.Find(linked_title);
			if (note != null)
				return string.Empty;

			try
			{
				note = _noteManager.Create(linked_title);
				note.QueueSave(ChangeType.ContentChanged);
				return note.Uri;
			}
			catch
		...
		public string CreateNamedNoteWithUri(string linked_title, string uri)
		{
			...
			try
			{
				note = _noteManager.CreateWithGuid(linked_title, guid);
				note.QueueSave(ChangeType.ContentChanged);
				return note.Uri;
			}
			catch
		...
		{
			NoteRecentChanges.GetInstance(_noteManager).Present();
		}

		public void DisplaySearchWithText(string search_text)
		{
			NoteRecentChanges recent_changes =
					NoteRecentChanges.GetInstance(_noteManager);
		...
		{
			Note note = _noteManager.FindByUri(uri);
			return note != null;
		}
		...
		public string[] ListAllNotes()
		{
			...
			foreach (Note note in _noteManager.Notes)
			{
				uris.Add(note.Uri);
			}
			return uris.ToArray();
		...
		public string[] SearchNotes(string query, bool case_sensitive)
		{
			...
			Search search = new Search(_noteManager);
